...
#include "PorousMediaSurfaceChemistryModel.H"
...
>
Foam::PorousMediaSurfaceChemistryModel
<GasReactionThermo, SolidReactionThermo, GasThermoType, SolidThermoType>::
PorousMediaSurfaceChemistryModel
(
...
:
    ...
    c_(nSpecie_+2),
    dcdt_(nSpecie_+2),
    FcatGeo_(this->lookupOrDefault("catalystToGeometricAreaRatio", 1.0)),
...
{ 
    ...
    forAll(siteDensityList_, i)
    {
        ...
        Info << "check: " << i << " ,siteDensityList[i] = " << siteDensityList_[i] << endl;
        Info << "check: " << i << " ,siteNumberList[i] = " << siteNumberList_[i] << endl;
    }



    Info << "    check size of catalyticSurfaces_ = " << catalyticSurfaces_.size() << endl;
    Info << "    check size of catalyticZones_ = " << catalyticZones_.size() << endl;
    ...
    {
        Info<< "    but not using maxIntegrationTime from catalystProperties dict \n" << endl;
    }
    ...
    Info<< "onlySurfaceCatalyst_ = "
        << onlySurfaceCatalyst_ << endl;
...
<
    ...
    class SolidThermoType
>
void Foam::PorousMediaSurfaceChemistryModel
<GasReactionThermo, SolidReactionThermo, GasThermoType, SolidThermoType>::
omega
(
    const scalar p,
...
<
    ...
    class SolidThermoType
>
Foam::scalar Foam::PorousMediaSurfaceChemistryModel
<GasReactionThermo, SolidReactionThermo, GasThermoType, SolidThermoType>::
omegaI
(
    const label index,
...
) const
{
    ...
    forAll(reactions_, ri)
    {
        ...
        scalar kfwd, kbwd;
        R.dwdc(p, T, c_, li, J, dcdt, omegaI, kfwd, kbwd, false, dummy, siteDensityList_, siteNumberList_);
    }
}
...
{
    //Info << "Nam check: using Solve(deltaT) function" << endl; //
    ...
    forAll(T, celli)
    {
        ...
        if (Ti > Treact_)
        {
           ...
           // update back Ys_ field after solving surface reactions
            ...
            {
                RRs_[i][celli] =
                    (c_[nGasSpecie_+i] - c0[nGasSpecie_+i])*specieThermos_[i].W()/deltaT[celli];
            }
...
<
    ...
    class SolidThermoType
>
Foam::scalarField Foam::PorousMediaSurfaceChemistryModel
<GasReactionThermo, SolidReactionThermo, GasThermoType, SolidThermoType>::
