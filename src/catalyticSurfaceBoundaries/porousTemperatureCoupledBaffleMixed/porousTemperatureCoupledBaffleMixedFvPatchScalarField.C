...
#include "porousTemperatureCoupledBaffleMixedFvPatchScalarField.H"
...
porousTemperatureCoupledBaffleMixedFvPatchScalarField::
porousTemperatureCoupledBaffleMixedFvPatchScalarField
(
    const fvPatch& p,
...
porousTemperatureCoupledBaffleMixedFvPatchScalarField
(
    const porousTemperatureCoupledBaffleMixedFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
:
    ...
    kappaLayers_(0),
    contactRes_(0.0)
    //keffMode_(dict.lookupOrDefault<word>("keffModel", "none")) //
...
porousTemperatureCoupledBaffleMixedFvPatchScalarField
(
    const porousTemperatureCoupledBaffleMixedFvPatchScalarField& wtcsf,
    const DimensionedField<scalar, volMesh>& iF
)
:
...
void porousTemperatureCoupledBaffleMixedFvPatchScalarField::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    typedef porousTemperatureCoupledBaffleMixedFvPatchScalarField thisType;
...
*/
    const fvPatchScalarField& nbrKappaEffPatch =
        nbrPatch.lookupPatchField<volScalarField, scalar>("kappaEff"); // JH

    if (contactRes_ == 0.0)
    {
        nbrIntFld.ref() = nbrField.patchInternalField();
        nbrKDelta.ref() = nbrKappaEffPatch*nbrPatch.deltaCoeffs(); // JH
    }
    else
    {
    ...
    // for my Field kappaEff
    const fvPatch& myPatch = patch(); // JH

    const fvPatchScalarField& kappaEffPatch =
        myPatch.lookupPatchField<volScalarField, scalar>("kappaEff"); // JH

    tmp<scalarField> myKDelta = kappaEffPatch*patch().deltaCoeffs(); // JH
...
void porousTemperatureCoupledBaffleMixedFvPatchScalarField::write
(
    Ostream& os
) const
{
